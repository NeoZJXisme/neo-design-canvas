...
+ [新增] 收藏节点支持评分和设计备注编辑，并可继续迭代或标记为工作流输出。
+ [新增] 模型渠道支持供应商、币种与单价配置，画布项目记录生成预估成本流水并在任务面板汇总。
+ [调整] 画布生成结果自动标记为 Output 工作流角色，继续复用现有图片、文本、视频和音频节点。

+ [新增] 画布左侧新增设计工作流快捷创建、收藏节点与生成任务摘要，并复用现有节点类型和聚焦交互。
+ [调整] 画布 Agent 显示层品牌化为 Creative Director / 创意总监，底层连接、协议与事件逻辑保持不变。
